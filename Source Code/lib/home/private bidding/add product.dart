...
class add_private_product extends StatefulWidget {
  final croomid;
  const add_private_product({super.key, this.croomid});

  @override
  State<add_private_product> createState() => _add_private_productState();
}

class _add_private_productState extends State<add_private_product> {
  ...
  Future<void> download() async {
    try {
      path = 'privateItemPhoto/${DateTime.now().millisecondsSinceEpoch}.jpeg';
  ...
  Future<void> _submitForm(BuildContext context) async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection("private rooms").doc(widget.croomid).collection("products").doc(DateTime.now().toString()).set(productData);
